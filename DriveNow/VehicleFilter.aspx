<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VehicleFilter.aspx.cs" Inherits="DriveNow.VehicleFilter" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Filter Vehicles — DriveNow Admin</title>
    ...
    <div class="dn-main">
        <div class="dn-topbar">
            ...
            <div class="dn-topbar-left">
                <span class="dn-page-title">Filter Vehicles</span>
                <span class="dn-page-sub">Filter the fleet by availability or date added</span>
            </div>
        ...
        <div class="dn-content">
            <div class="dn-form-card" style="margin-bottom:24px;">
                <div class="dn-field">
                    <label class="dn-label">Availability Status</label>
                    <asp:DropDownList ID="ddlAvailability" runat="server" CssClass="dn-select">
                        <asp:ListItem Text="All Vehicles"                   Value=""  />
                        <asp:ListItem Text="Active only (IsAvailable = 1)"  Value="1" />
                        <asp:ListItem Text="Removed only (IsAvailable = 0)" Value="0" />
                    </asp:DropDownList>
                    <span class="dn-hint">Active = available for booking. Removed = soft deleted.</span>
                </div>
                <div class="dn-field">
                    <label class="dn-label">Added to Fleet From</label>
                    <asp:TextBox ID="txtDateFrom" runat="server" CssClass="dn-input" TextMode="Date" />
                    <span class="dn-hint">Only show vehicles added on or after this date. Leave blank for all dates.</span>
                </div>
                <div class="dn-form-actions">
                    <asp:Button ID="btnFilter" runat="server" Text="Apply Filter" CssClass="dn-btn dn-btn-primary"   OnClick="btnFilter_Click" />
                    <asp:Button ID="btnClear"  runat="server" Text="Clear Filter" CssClass="dn-btn dn-btn-secondary" OnClick="btnClear_Click" />
                </div>
            </div>

            <asp:Label ID="lblResultCount" runat="server" Text="" Visible="false" CssClass="dn-alert-success" />
            <asp:Label ID="lblNoResults"   runat="server" Text="" Visible="false" CssClass="dn-alert-error" />
                    ...
                    <asp:GridView ID="gvResults" runat="server"
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="Daily Rate">
                                <ItemTemplate>£<%# string.Format("{0:N2}", Eval("DailyRate")) %></ItemTemplate>
                            </asp:TemplateField>
                            ...
                            <asp:TemplateField HeaderText="Status">
                                <ItemTemplate>
                                    <%# Convert.ToBoolean(Eval("IsAvailable"))
                                        ...
                                        : "<span class=\"dn-status\"><span class=\"dn-dot-red\"></span> Removed</span>" %>
                                </ItemTemplate>
                            </asp:TemplateField>
                            <asp:TemplateField HeaderText="Actions">
                                <ItemTemplate>
                                    <a href='VehicleEdit.aspx?id=<%# Eval("VehicleID") %>' class="dn-action-edit">Edit</a>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
